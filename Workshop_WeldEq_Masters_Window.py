...
class Ui_Workshop_WeldEq_Masters_Window(QtWidgets.QMainWindow):
    """
    ...
    def closeConnection(self):
        ...
        """
        self.tableWeldEqMasters.setModel(None)
        del self.model
        if self.db:
            self.db.close()
            del self.db
    ...
    def setupUi(self, Workshop_WeldEq_Masters_Window):
        """
        Sets up the user interface for the Workshop_WeldEq_Masters_Window.

        Args:
            Workshop_WeldEq_Masters_Window (QtWidgets.QMainWindow): The main window for the UI setup.
        """
        Workshop_WeldEq_Masters_Window.setObjectName("Workshop_WeldEq_Masters_Window")
        Workshop_WeldEq_Masters_Window.resize(790, 595)
        Workshop_WeldEq_Masters_Window.setMinimumSize(QtCore.QSize(790, 595))
        icon = QtGui.QIcon()
        icon.addPixmap(QtGui.QPixmap(os.path.abspath(os.path.join(basedir, "Resources/Iconos/icon.ico"))), QtGui.QIcon.Mode.Normal, QtGui.QIcon.State.Off)
        Workshop_WeldEq_Masters_Window.setWindowIcon(icon)
        if self.username == 'm.gil':
            Workshop_WeldEq_Masters_Window.setStyleSheet(
            ...
            "}\n"
            "QMenu::item:selected {background-color: rgb(3, 174, 236);}")
        self.centralwidget = QtWidgets.QWidget(parent=Workshop_WeldEq_Masters_Window)
        if self.username == 'm.gil':
        ...
        self.tableWeldEqMasters=QtWidgets.QTableView(parent=self.frame)
        self.model = EditableTableModel()
        self.tableWeldEqMasters.setObjectName("tableWeldEqMasters")
        self.gridLayout_2.addWidget(self.tableWeldEqMasters, 2, 0, 1, 1)
        ...
        Workshop_WeldEq_Masters_Window.setCentralWidget(self.centralwidget)
        self.menubar = QtWidgets.QMenuBar(parent=Workshop_WeldEq_Masters_Window)
        self.menubar.setGeometry(QtCore.QRect(0, 0, 790, 22))
        self.menubar.setObjectName("menubar")
        Workshop_WeldEq_Masters_Window.setMenuBar(self.menubar)
        self.statusbar = QtWidgets.QStatusBar(parent=Workshop_WeldEq_Masters_Window)
        self.statusbar.setObjectName("statusbar")
        Workshop_WeldEq_Masters_Window.setStatusBar(self.statusbar)

        self.retranslateUi(Workshop_WeldEq_Masters_Window)
        QtCore.QMetaObject.connectSlotsByName(Workshop_WeldEq_Masters_Window)
        ...
        self.tableWeldEqMasters.setContextMenuPolicy(Qt.ContextMenuPolicy.ActionsContextMenu)
        self.tableWeldEqMasters.addActions([delete_action])
...
# Function to translate and updates the text of various UI elements
    def retranslateUi(self, Workshop_WeldEq_Masters_Window):
        """
        Translates and updates the text of various UI elements.
        """
        _translate = QtCore.QCoreApplication.translate
        Workshop_WeldEq_Masters_Window.setWindowTitle(_translate("Workshop_WeldEq_Masters_Window", "Equipos Soldadura"))
        self.tableWeldEqMasters.setSortingEnabled(True)
...
# Function to delete all filters when tool button is clicked
    def delete_allFilters(self):
        ...
        if self.proxy.rowCount() != 0:
            ...
            self.tableWeldEqMasters.setModel(None)
            self.tableWeldEqMasters.setModel(self.proxy)
...
# Function to load filtered table and setting in the window
    def query_masters(self):
        """
        Queries the database for calibrated masters with filter applied, configures and populates tables with the query results, 
        ...
        self.model.setTable("verification.welding_equipment_masters")
        ...
        self.tableWeldEqMasters.setItemDelegate(AlignDelegate(self.tableWeldEqMasters))
        # self.color_delegate = ColorDelegate(self)
        # self.tableWeldEqMasters.setItemDelegateForColumn(5, self.color_delegate)

        if self.username == 'm.gil':
            self.tableWeldEqMasters.setStyleSheet("gridline-color: rgb(128, 128, 128);")
            self.tableWeldEqMasters.horizontalHeader().setStyleSheet("::section{font: 800 10pt; background-color: #33bdef; border: 1px solid white;}")
            self.tableWeldEqMasters.verticalHeader().setStyleSheet("::section{font: 10pt; background-color: #121212; border: 0.5px solid white;}")
        else:
            self.tableWeldEqMasters.horizontalHeader().setStyleSheet("::section{font: 800 10pt; background-color: #33bdef; border: 1px solid black;}")

        self.tableWeldEqMasters.setObjectName("tableWeldEqMasters")
        self.gridLayout_2.addWidget(self.tableWeldEqMasters, 2, 0, 1, 1)
        self.tableWeldEqMasters.setSortingEnabled(False)
        self.tableWeldEqMasters.verticalHeader().setVisible(False)
    ...
    # Getting the unique values for each column of the model
        ...
        self.selection_model = self.tableWeldEqMasters.selectionModel()
        self.selection_model.selectionChanged.connect(self.countSelectedCells)

        self.tableWeldEqMasters.horizontalHeader().sectionDoubleClicked.connect(self.on_view_horizontalHeader_sectionClicked)
        self.tableWeldEqMasters.horizontalHeader().customContextMenuRequested.connect(self.showColumnContextMenu)
        self.tableWeldEqMasters.horizontalHeader().setContextMenuPolicy(Qt.ContextMenuPolicy.CustomContextMenu)

        self.tableWeldEqMasters.sortByColumn(0, Qt.SortOrder.AscendingOrder)

        self.tableWeldEqMasters.doubleClicked.connect(lambda index: self.item_double_clicked(index))
...
# Function when header is clicked
    def on_view_horizontalHeader_sectionClicked(self, logicalIndex):
        ...
        self.signalMapper = QtCore.QSignalMapper(self.tableWeldEqMasters)

        valuesUnique_view = []
        for row in range(self.tableWeldEqMasters.model().rowCount()):
            index = self.tableWeldEqMasters.model().index(row, self.logicalIndex)
        ...
        actionSortAscending = QtGui.QAction("Ordenar Ascendente", self.tableWeldEqMasters)
        actionSortAscending.triggered.connect(self.on_actionSortAscending_triggered)
        self.menuValues.addAction(actionSortAscending)
        actionSortDescending = QtGui.QAction("Ordenar Descendente", self.tableWeldEqMasters)
        ...
        actionDeleteFilterColumn = QtGui.QAction("Quitar Filtro", self.tableWeldEqMasters)
        ...
        actionTextFilter = QtGui.QAction("Buscar...", self.tableWeldEqMasters)
        ...
        accept_button = QtGui.QAction("ACEPTAR", self.tableWeldEqMasters)
        accept_button.triggered.connect(self.menu_acceptbutton_triggered)

        cancel_button = QtGui.QAction("CANCELAR", self.tableWeldEqMasters)
        ...
        headerPos = self.tableWeldEqMasters.mapToGlobal(self.tableWeldEqMasters.horizontalHeader().pos())        

        posY = headerPos.y() + self.tableWeldEqMasters.horizontalHeader().height()
        scrollX = self.tableWeldEqMasters.horizontalScrollBar().value()
        xInView = self.tableWeldEqMasters.horizontalHeader().sectionViewportPosition(logicalIndex)
...
# Function to hide column when action clicked
    def hide_column(self):
        ...
        """
        filterColumn = self.logicalIndex
        self.tableWeldEqMasters.setColumnHidden(filterColumn, True)
...
# Function to show all hidden columns
    def show_columns(self):
        ...
        for column in self.hiddencolumns:
            self.tableWeldEqMasters.setColumnHidden(column, False)
...
# Function to enable copy and paste cells
    def keyPressEvent(self, event):
        ...
        if event.matches(QKeySequence.StandardKey.Copy):
            if self.tableWeldEqMasters.selectionModel() != None:
                selected_indexes = self.tableWeldEqMasters.selectionModel().selectedIndexes()
                if selected_indexes:
...
# Function to count selected cells and sum its values
    def countSelectedCells(self):
        ...
        """
        if len(self.tableWeldEqMasters.selectedIndexes()) > 1:
            ...
            sum_value = sum([self.euro_string_to_float(str(ix.data())) if re.match(r'^[\d.,]+\sÇ$', str(ix.data())) else float(str(ix.data()).replace(',', '.')) if str(ix.data()).replace(',', '.').replace('.', '', 1).isdigit() else 0 for ix in self.tableWeldEqMasters.selectedIndexes()])
            count_value = len([ix for ix in self.tableWeldEqMasters.selectedIndexes() if ix.data() != ""])
            if sum_value > 0:
...
# Function to show context menu when right-click
    def showColumnContextMenu(self, pos):
        ...
        """
        header = self.tableWeldEqMasters.horizontalHeader()
        column = header.logicalIndexAt(pos)
        self.context_menu.exec(self.tableWeldEqMasters.mapToGlobal(pos))
...
# Function to hide selected columns
    def hideSelectedColumns(self):
        ...
        for column in selected_columns:
            self.tableWeldEqMasters.setColumnHidden(column, True)
...
# Function to add a new line
    def add_new(self):
        """
        Inserts a new empty entry into the welding_equipment_masters table.
        Commits the changes to the database and handles any errors.
        """
        commands_new=("""
                        INSERT INTO verification.welding_equipment_masters (number_item)
                        VALUES(%s)
                        """)
        conn = None
        try:
...
# Function to adjust table size
    def adjust_table(self):
        """
        Adjusts column visibility and resize behavior in the tableWeldEqMasters widget.
        """
        self.tableWeldEqMasters.horizontalHeader().setSectionResizeMode(QtWidgets.QHeaderView.ResizeMode.ResizeToContents)
        self.tableWeldEqMasters.horizontalHeader().setSectionResizeMode(2,QtWidgets.QHeaderView.ResizeMode.Stretch)
        self.tableWeldEqMasters.horizontalHeader().setSectionResizeMode(3,QtWidgets.QHeaderView.ResizeMode.Stretch)
        self.tableWeldEqMasters.horizontalHeader().setSectionResizeMode(7,QtWidgets.QHeaderView.ResizeMode.Stretch)
        self.tableWeldEqMasters.verticalHeader().setSectionResizeMode(QtWidgets.QHeaderView.ResizeMode.ResizeToContents)
        self.tableWeldEqMasters.verticalHeader().setVisible(False)
        self.tableWeldEqMasters.hideColumn(8)
...
# Function to delete register of database
    def delete_register(self):
        ...
        """
        selection_model = self.tableWeldEqMasters.selectionModel()

        if not selection_model.hasSelection():
            return

        model = self.tableWeldEqMasters.model()
    ...
    def add_pdf(self):
        ...
        if len(selected_indexes) == 1:
            ...
            if index.column() in [2, 3]:
                ...
                if pdf_path:
                    commands_insert_1 = ("""
                            UPDATE verification."welding_equipment_masters"
                            SET "certificate_1" = %s
                            WHERE "number_item" = %s
                            """)
...
# Function to open menu to export data
    def export_data(self):
        ...
        if self.proxy.rowCount() == 0:
            ...
            visible_columns = [col for col in range(self.model.columnCount()) if not self.tableWeldEqMasters.isColumnHidden(col)]
...
if __name__ == "__main__":
    ...
    Workshop_WeldEq_Masters_Window = Ui_Workshop_WeldEq_Masters_Window(db, 'm.gil')
    Workshop_WeldEq_Masters_Window.showMaximized()
    sys.exit(app.exec())