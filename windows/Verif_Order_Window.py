        ...
        if item.text() != '':
            if item.column() == 2: # Msgbox for tag
                ...
                if self.num_order_value[:2] == 'PA':
                    ...
                    path = "//ERP-EIPSA-DATOS/base de datos de pedidos/Año " + order_year + "/" + order_year + " Pedidos Almacen"
                    for folder in os.listdir(path):
                        if num_order.replace("/", "-") in folder:
                            folder_path = "//ERP-EIPSA-DATOS/base de datos de pedidos/Año " + order_year + "/" + order_year + " Pedidos Almacen/" + folder + "/"
                    ...
                    path = "//ERP-EIPSA-DATOS/base de datos de pedidos/Año " + order_year + "/" + order_year + " Pedidos"
                    for folder in os.listdir(path):
                        if num_order.replace("/", "-") in folder:
                            folder_path = "//ERP-EIPSA-DATOS/base de datos de pedidos/Año " + order_year + "/" + order_year + " Pedidos/" + folder + "/"
                            for root, dirs, files in os.walk(folder_path):
                                for filename in files:
                                    if fnmatch.fnmatch(filename, '*-PPI*'):
                                        file_path = os.path.join(root, filename)
