# Form implementation generated from reading ui file 'ReportClientOrderResume_Window.ui'
#
...
class Ui_ClientOrderResume_Window(object):
    def __init__(self, username, ref_order = None):
        """
        Initializes the Ui_ClientOrderResume_Window with the specified name and username.

        Args:
            name (str): name associated with the window.
            username (str): username associated with the window.
            ref_order (str, optional): supply reference to check data. Defaults to None
        """
        self.username=username
        self.ref_order = ref_order

    def setupUi(self, ClientOrderResume):
        if self.username in ['d.marquez']:
            self.scale = 1.5
        else:
            self.scale=1
        ClientOrderResume.setObjectName("ClientOrderResume")
    ...
    "}"
        ...
        self.label_order.setMinimumSize(QtCore.QSize(int(100//self.scale), int(35//self.scale)))
        self.label_order.setMaximumSize(QtCore.QSize(int(100//self.scale), int(35//self.scale)))
        font = QtGui.QFont()
        font.setPointSize(int(14//self.scale))
        font.setBold(True)
        self.label_order.setFont(font)
        self.label_order.setObjectName("label_order")
        self.gridLayout1.addWidget(self.label_order, 0, 0, 1, 1)
        self.Num_Order = QtWidgets.QLabel(parent=self.frame)
        self.Num_Order.setMinimumSize(QtCore.QSize(0, int(35//self.scale)))
        self.Num_Order.setMaximumSize(QtCore.QSize(16777215, int(35//self.scale)))
        font = QtGui.QFont()
        font.setPointSize(int(14//self.scale))
        self.Num_Order.setFont(font)
        self.Num_Order.setObjectName("Num_Order")
        self.gridLayout1.addWidget(self.Num_Order, 0, 1, 1, 2)
        ...
        ClientOrderResume.setCentralWidget(self.centralwidget)
        self.menubar = QtWidgets.QMenuBar(parent=ClientOrderResume)
        self.menubar.setGeometry(QtCore.QRect(0, 0, 1165, 22))
        self.menubar.setObjectName("menubar")
        ClientOrderResume.setMenuBar(self.menubar)
        self.statusbar = QtWidgets.QStatusBar(parent=ClientOrderResume)
        self.statusbar.setObjectName("statusbar")
        ClientOrderResume.setStatusBar(self.statusbar)
        ...
        self.retranslateUi(ClientOrderResume)
        QtCore.QMetaObject.connectSlotsByName(ClientOrderResume)

        if self.ref_order is not None:
            commands_numorder = "SELECT client_order_num FROM purch_fact.client_ord_header WHERE id = %s"
            conn = None
            try:
            ...
            # execution of commands one by one
                cur.execute(commands_numorder, (self.ref_order,))
                results_num_order=cur.fetchall()
            ...
            # commit the changes
                ...
                if conn is not None:
                    conn.close()
            self.Num_Order.setText(results_num_order[0][0])
            self.loaddata(self.ref_order)

        self.tableWidget.horizontalHeader().sectionClicked.connect(self.on_header_section_clicked)
        self.tableWidget.itemSelectionChanged.connect(self.countSelectedCells)

# Function to translate and updates the text of various UI elements
    def retranslateUi(self, ClientOrderResume):
        """
        Translates and updates the text of various UI elements.
        """
        _translate = QtCore.QCoreApplication.translate
        ClientOrderResume.setWindowTitle(_translate("ClientOrderResume", "Resumen Pedido"))
        item = self.tableWidget.horizontalHeaderItem(0)
        item.setText(_translate("ClientOrderResume", "Insumo"))
        item = self.tableWidget.horizontalHeaderItem(1)
        item.setText(_translate("ClientOrderResume", "Descripción"))
        item = self.tableWidget.horizontalHeaderItem(2)
        item.setText(_translate("ClientOrderResume", "Stock"))
        item = self.tableWidget.horizontalHeaderItem(3)
        item.setText(_translate("ClientOrderResume", "Stock Dsp."))
        item = self.tableWidget.horizontalHeaderItem(4)
        item.setText(_translate("ClientOrderResume", "Cantidad"))
        item = self.tableWidget.horizontalHeaderItem(5)
        item.setText(_translate("ClientOrderResume", "Pendiente"))
        item = self.tableWidget.horizontalHeaderItem(6)
        item.setText(_translate("ClientOrderResume", "Entrega 1"))
        item = self.tableWidget.horizontalHeaderItem(7)
        item.setText(_translate("ClientOrderResume", "Entrega 2"))
        item = self.tableWidget.horizontalHeaderItem(8)
        item.setText(_translate("ClientOrderResume", "Entrega 3"))
        self.label_order.setText(_translate("ClientOrderResume", "Pedido"))

# Function to load data in table
    def loaddata(self, order_id):
        """
        ...
        commands_querytable = ("""
                        SELECT purch_fact.supplies."reference", purch_fact.supplies."description",
        ...
        # connect to the PostgreSQL server
            ...
            cur = conn.cursor()
        # execution of commands one by one
            cur.execute(commands_querytable,(order_id,))
...
# Function to count selected cell and sum its values if possible
    def countSelectedCells(self):
        ...
        if len(self.tableWidget.selectedIndexes()) > 1:
            ...
            sum_value = sum([(float(ix.data()) if (ix.data() is not None and ix.data().replace(',', '.', 1).replace('.', '', 1).isdigit() and ix.column() > 1) else 0) for ix in self.tableWidget.selectedIndexes()])
            count_value = len([ix for ix in self.tableWidget.selectedIndexes() if ix.data() != ""])
            if sum_value > 0:
...
if __name__ == "__main__":
    ...
    ClientOrderResume = QtWidgets.QMainWindow()
    ui = Ui_ClientOrderResume_Window('d.marquez','2953')
    ui.setupUi(ClientOrderResume)
    ClientOrderResume.show()
    sys.exit(app.exec())
