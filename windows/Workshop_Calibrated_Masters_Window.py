...
class Ui_Workshop_Calibrated_Masters_Window(QtWidgets.QMainWindow):
    ...
    def setupUi(self, Workshop_Calibrated_Masters_Window):
        """
        Sets up the user interface for the Workshop_Calibrated_Masters_Window.

        Args:
            Workshop_Calibrated_Masters_Window (QtWidgets.QMainWindow): The main window for the UI setup.
        """
        Workshop_Calibrated_Masters_Window.setObjectName("Workshop_Calibrated_Masters_Window")
        ...
        self.tableCalibratedMasters=QtWidgets.QTableView(parent=self.frame)
        self.model = EditableTableModel()
        ...
        Workshop_Calibrated_Masters_Window.setCentralWidget(self.centralwidget)
        self.menubar = QtWidgets.QMenuBar(parent=Workshop_Calibrated_Masters_Window)
        self.menubar.setGeometry(QtCore.QRect(0, 0, 790, 22))
        self.menubar.setObjectName("menubar")
        Workshop_Calibrated_Masters_Window.setMenuBar(self.menubar)
        self.statusbar = QtWidgets.QStatusBar(parent=Workshop_Calibrated_Masters_Window)
        self.statusbar.setObjectName("statusbar")
        Workshop_Calibrated_Masters_Window.setStatusBar(self.statusbar)

        self.retranslateUi(Workshop_Calibrated_Masters_Window)
        QtCore.QMetaObject.connectSlotsByName(Workshop_Calibrated_Masters_Window)
        ...
        self.tableCalibratedMasters.setContextMenuPolicy(Qt.ContextMenuPolicy.ActionsContextMenu)
        self.tableCalibratedMasters.addActions([delete_action])
...
# Function to translate and updates the text of various UI elements
    def retranslateUi(self, Workshop_Calibrated_Masters_Window):
        """
        Translates and updates the text of various UI elements.
        """
        _translate = QtCore.QCoreApplication.translate
        Workshop_Calibrated_Masters_Window.setWindowTitle(_translate("Workshop_Calibrated_Masters_Window", "Patrones Calibrados"))
        self.tableCalibratedMasters.setSortingEnabled(True)
...
# Function to load filtered table and setting in the window
    def query_masters(self):
        ...
        self.tableCalibratedMasters.horizontalHeader().setStyleSheet("::section{font: 800 10pt; background-color: #33bdef; border: 1px solid;}")
...
# Function when header is clicked
    def on_view_horizontalHeader_sectionClicked(self, logicalIndex):
        ...
        self.signalMapper = QtCore.QSignalMapper(self.tableCalibratedMasters)

        valuesUnique_view = []
        for row in range(self.tableCalibratedMasters.model().rowCount()):
            index = self.tableCalibratedMasters.model().index(row, self.logicalIndex)
        ...
        actionSortAscending = QtGui.QAction("Ordenar Ascendente", self.tableCalibratedMasters)
        actionSortAscending.triggered.connect(self.on_actionSortAscending_triggered)
        self.menuValues.addAction(actionSortAscending)
        actionSortDescending = QtGui.QAction("Ordenar Descendente", self.tableCalibratedMasters)
        ...
        actionDeleteFilterColumn = QtGui.QAction("Quitar Filtro", self.tableCalibratedMasters)
        ...
        actionTextFilter = QtGui.QAction("Buscar...", self.tableCalibratedMasters)
        ...
        accept_button = QtGui.QAction("ACEPTAR", self.tableCalibratedMasters)
        accept_button.triggered.connect(self.menu_acceptbutton_triggered)

        cancel_button = QtGui.QAction("CANCELAR", self.tableCalibratedMasters)
        ...
        headerPos = self.tableCalibratedMasters.mapToGlobal(self.tableCalibratedMasters.horizontalHeader().pos())        

        posY = headerPos.y() + self.tableCalibratedMasters.horizontalHeader().height()
        scrollX = self.tableCalibratedMasters.horizontalScrollBar().value()
        xInView = self.tableCalibratedMasters.horizontalHeader().sectionViewportPosition(logicalIndex)
...
# Function to hide column when action clicked
    def hide_column(self):
        ...
        """
        filterColumn = self.logicalIndex
        self.tableCalibratedMasters.setColumnHidden(filterColumn, True)
...
# Function to enable copy and paste cells
    def keyPressEvent(self, event):
        ...
        if event.matches(QKeySequence.StandardKey.Copy):
            if self.tableCalibratedMasters.selectionModel() != None:
...
# Function to count selected cells and sum its values
    def countSelectedCells(self):
        ...
        """
        if len(self.tableCalibratedMasters.selectedIndexes()) > 1:
            ...
            sum_value = sum([self.euro_string_to_float(str(ix.data())) if re.match(r'^[\d.,]+\sÇ$', str(ix.data())) else float(str(ix.data()).replace(',', '.')) if str(ix.data()).replace(',', '.').replace('.', '', 1).isdigit() else 0 for ix in self.tableCalibratedMasters.selectedIndexes()])
            count_value = len([ix for ix in self.tableCalibratedMasters.selectedIndexes() if ix.data() != ""])
            if sum_value > 0:
...
# Function to show context menu when right-click
    def showColumnContextMenu(self, pos):
        ...
        """
        header = self.tableCalibratedMasters.horizontalHeader()
        column = header.logicalIndexAt(pos)
        self.context_menu.exec(self.tableCalibratedMasters.mapToGlobal(pos))
...
# Function to hide selected columns
    def hideSelectedColumns(self):
        ...
        for column in selected_columns:
            self.tableCalibratedMasters.setColumnHidden(column, True)
...
# Function to add a new line
    def add_new(self):
        """
        Inserts a new empty entry into the calibrated_masters table.
...
# Function to delete register of database
    def delete_register(self):
        ...
        """
        selection_model = self.tableCalibratedMasters.selectionModel()

        if not selection_model.hasSelection():
            return

        model = self.tableCalibratedMasters.model()
    ...
    def add_pdf(self):
        ...
        if len(selected_indexes) == 1:
            ...
            if index.column() in [2, 3]:
                ...
                pdf_path, _ = QtWidgets.QFileDialog.getOpenFileName(None, "Seleccionar PDF", initialdir, "Archivos PDF (*.pdf)")
