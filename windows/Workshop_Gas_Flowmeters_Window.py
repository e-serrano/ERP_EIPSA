...
class Ui_Workshop_Gas_Flowmeters_Window(QtWidgets.QMainWindow):
    ...
    def setupUi(self, Workshop_Gas_Flowmeters_Window):
        """
        Sets up the user interface for the Workshop_Gas_Flowmeters_Window.

        Args:
            Workshop_Gas_Flowmeters_Window (QtWidgets.QMainWindow): The main window for the UI setup.
        """
        Workshop_Gas_Flowmeters_Window.setObjectName("Workshop_Gas_Flowmeters_Window")
        Workshop_Gas_Flowmeters_Window.resize(790, 595)
        Workshop_Gas_Flowmeters_Window.setMinimumSize(QtCore.QSize(790, 595))
        icon = QtGui.QIcon()
        ...
        Workshop_Gas_Flowmeters_Window.setCentralWidget(self.centralwidget)
        self.menubar = QtWidgets.QMenuBar(parent=Workshop_Gas_Flowmeters_Window)
        self.menubar.setGeometry(QtCore.QRect(0, 0, 790, 22))
        self.menubar.setObjectName("menubar")
        Workshop_Gas_Flowmeters_Window.setMenuBar(self.menubar)
        self.statusbar = QtWidgets.QStatusBar(parent=Workshop_Gas_Flowmeters_Window)
        self.statusbar.setObjectName("statusbar")
        Workshop_Gas_Flowmeters_Window.setStatusBar(self.statusbar)

        self.retranslateUi(Workshop_Gas_Flowmeters_Window)
        QtCore.QMetaObject.connectSlotsByName(Workshop_Gas_Flowmeters_Window)
...
# Function to translate and updates the text of various UI elements
    def retranslateUi(self, Workshop_Gas_Flowmeters_Window):
        """
        Translates and updates the text of various UI elements.
        """
        _translate = QtCore.QCoreApplication.translate
        Workshop_Gas_Flowmeters_Window.setWindowTitle(_translate("Workshop_Gas_Flowmeters_Window", "Medidores de Caudal de Gas Taller"))
...
# Function to load filtered table and setting in the window
    def query_equipment(self):
        ...
        self.model.setTable("verification.gas_flowmeters_workshop")
    ...
    # Change all column names
        headers_names = ["ID", "Equipo", "Situación", "Marca",
                        "Unidades", "Patrón", "Última Rev.", "Prox Rev.", "Notas"]
...
# Function to count selected cells and sum its values
    def countSelectedCells(self):
        ...
        if len(self.tableEquipment.selectedIndexes()) > 1:
            ...
            sum_value = sum([self.euro_string_to_float(str(ix.data())) if re.match(r'^[\d.,]+\sÇ$', str(ix.data())) else float(str(ix.data()).replace(',', '.')) if str(ix.data()).replace(',', '.').replace('.', '', 1).isdigit() else 0 for ix in self.tableEquipment.selectedIndexes()])
...
# Function to add a new line
    def add_new(self):
        ...
        """
        commands_new=("""
                        INSERT INTO verification.gas_flowmeters_workshop (equipment)
                        VALUES(%s)
                        """)
        conn = None
        try:
...
# Function to see revisions of equipment
    def open_equipment_information(self,index):
        ...
        from windows.Workshop_Gas_Flowmeters_Rev_Window import Ui_Workshop_Gas_Flowmeters_Rev_Window
        ...
        self.equipment_rev_window = Ui_Workshop_Gas_Flowmeters_Rev_Window(db_equipment, self.username, value)
        ...
        if len(id_values) != 0:
            ...
            if result == QtWidgets.QMessageBox.StandardButton.Yes:
                ...
                # execution of commands
                    commands_delete = ("""DELETE FROM verification.Gas_Flowmeters_workshop
                                        WHERE id = %s""")
...
if __name__ == "__main__":
    ...
    Workshop_Gas_Flowmeters_Window = Ui_Workshop_Gas_Flowmeters_Window(db, 'm.gil')
    Workshop_Gas_Flowmeters_Window.showMaximized()
    sys.exit(app.exec())