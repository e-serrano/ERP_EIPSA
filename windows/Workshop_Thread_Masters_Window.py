...
class Ui_Workshop_Thread_Masters_Window(QtWidgets.QMainWindow):
    """
    ...
    def closeConnection(self):
        ...
        """
        self.tableThreadMasters.setModel(None)
        del self.model
        if self.db:
            self.db.close()
            del self.db
    ...
    def setupUi(self, Workshop_Thread_Masters_Window):
        """
        Sets up the user interface for the Workshop_Thread_Masters_Window.

        Args:
            Workshop_Thread_Masters_Window (QtWidgets.QMainWindow): The main window for the UI setup.
        """
        Workshop_Thread_Masters_Window.setObjectName("Workshop_Thread_Masters_Window")
        Workshop_Thread_Masters_Window.resize(790, 595)
        Workshop_Thread_Masters_Window.setMinimumSize(QtCore.QSize(790, 595))
        icon = QtGui.QIcon()
        ...
        self.tableThreadMasters=QtWidgets.QTableView(parent=self.frame)
        self.model = EditableTableModel()
        ...
        Workshop_Thread_Masters_Window.setCentralWidget(self.centralwidget)
        self.menubar = QtWidgets.QMenuBar(parent=Workshop_Thread_Masters_Window)
        self.menubar.setGeometry(QtCore.QRect(0, 0, 790, 22))
        self.menubar.setObjectName("menubar")
        Workshop_Thread_Masters_Window.setMenuBar(self.menubar)
        self.statusbar = QtWidgets.QStatusBar(parent=Workshop_Thread_Masters_Window)
        self.statusbar.setObjectName("statusbar")
        Workshop_Thread_Masters_Window.setStatusBar(self.statusbar)

        self.retranslateUi(Workshop_Thread_Masters_Window)
        QtCore.QMetaObject.connectSlotsByName(Workshop_Thread_Masters_Window)
        ...
        self.tableThreadMasters.setContextMenuPolicy(Qt.ContextMenuPolicy.ActionsContextMenu)
        self.tableThreadMasters.addActions([delete_action])
...
# Function to translate and updates the text of various UI elements
    def retranslateUi(self, Workshop_Thread_Masters_Window):
        """
        Translates and updates the text of various UI elements.
        """
        _translate = QtCore.QCoreApplication.translate
        Workshop_Thread_Masters_Window.setWindowTitle(_translate("Workshop_Thread_Masters_Window", "Patrones Roscas"))
        self.tableThreadMasters.setSortingEnabled(True)
...
# Function to delete all filters when tool button is clicked
    def delete_allFilters(self):
        ...
        if self.proxy.rowCount() != 0:
            ...
            self.tableThreadMasters.setModel(None)
            self.tableThreadMasters.setModel(self.proxy)
...
# Function to load filtered table and setting in the window
    def query_masters(self):
        ...
        self.model.setTable("verification.thread_masters")
        ...
        self.tableThreadMasters.setObjectName("tableThreadMasters")
        self.gridLayout_2.addWidget(self.tableThreadMasters, 2, 0, 1, 1)
        self.tableThreadMasters.setSortingEnabled(False)
        self.tableThreadMasters.verticalHeader().setVisible(False)
    ...
    # Getting the unique values for each column of the model
        ...
        self.selection_model = self.tableThreadMasters.selectionModel()
        self.selection_model.selectionChanged.connect(self.countSelectedCells)

        self.tableThreadMasters.horizontalHeader().sectionDoubleClicked.connect(self.on_view_horizontalHeader_sectionClicked)
        self.tableThreadMasters.horizontalHeader().customContextMenuRequested.connect(self.showColumnContextMenu)
        self.tableThreadMasters.horizontalHeader().setContextMenuPolicy(Qt.ContextMenuPolicy.CustomContextMenu)

        self.tableThreadMasters.sortByColumn(0, Qt.SortOrder.AscendingOrder)

        self.tableThreadMasters.doubleClicked.connect(lambda index: self.item_double_clicked(index))
...
# Function when header is clicked
    def on_view_horizontalHeader_sectionClicked(self, logicalIndex):
        ...
        self.signalMapper = QtCore.QSignalMapper(self.tableThreadMasters)

        valuesUnique_view = []
        for row in range(self.tableThreadMasters.model().rowCount()):
            index = self.tableThreadMasters.model().index(row, self.logicalIndex)
        ...
        actionSortAscending = QtGui.QAction("Ordenar Ascendente", self.tableThreadMasters)
        actionSortAscending.triggered.connect(self.on_actionSortAscending_triggered)
        self.menuValues.addAction(actionSortAscending)
        actionSortDescending = QtGui.QAction("Ordenar Descendente", self.tableThreadMasters)
        ...
        actionDeleteFilterColumn = QtGui.QAction("Quitar Filtro", self.tableThreadMasters)
        ...
        actionTextFilter = QtGui.QAction("Buscar...", self.tableThreadMasters)
        ...
        accept_button = QtGui.QAction("ACEPTAR", self.tableThreadMasters)
        accept_button.triggered.connect(self.menu_acceptbutton_triggered)

        cancel_button = QtGui.QAction("CANCELAR", self.tableThreadMasters)
        ...
        posY = headerPos.y() + self.tableThreadMasters.horizontalHeader().height()
        scrollX = self.tableThreadMasters.horizontalScrollBar().value()
        xInView = self.tableThreadMasters.horizontalHeader().sectionViewportPosition(logicalIndex)
...
# Function to hide column when action clicked
    def hide_column(self):
        ...
        """
        filterColumn = self.logicalIndex
        self.tableThreadMasters.setColumnHidden(filterColumn, True)
...
# Function to show all hidden columns
    def show_columns(self):
        ...
        for column in self.hiddencolumns:
            self.tableThreadMasters.setColumnHidden(column, False)
...
# Function to enable copy and paste cells
    def keyPressEvent(self, event):
        ...
        if event.matches(QKeySequence.StandardKey.Copy):
            if self.tableThreadMasters.selectionModel() != None:
                selected_indexes = self.tableThreadMasters.selectionModel().selectedIndexes()
                if selected_indexes:
...
# Function to count selected cells and sum its values
    def countSelectedCells(self):
        ...
        """
        if len(self.tableThreadMasters.selectedIndexes()) > 1:
            ...
            sum_value = sum([self.euro_string_to_float(str(ix.data())) if re.match(r'^[\d.,]+\sÇ$', str(ix.data())) else float(str(ix.data()).replace(',', '.')) if str(ix.data()).replace(',', '.').replace('.', '', 1).isdigit() else 0 for ix in self.tableThreadMasters.selectedIndexes()])
            count_value = len([ix for ix in self.tableThreadMasters.selectedIndexes() if ix.data() != ""])
            if sum_value > 0:
...
# Function to show context menu when right-click
    def showColumnContextMenu(self, pos):
        ...
        """
        header = self.tableThreadMasters.horizontalHeader()
        column = header.logicalIndexAt(pos)
        self.context_menu.exec(self.tableThreadMasters.mapToGlobal(pos))
...
# Function to hide selected columns
    def hideSelectedColumns(self):
        ...
        for column in selected_columns:
            self.tableThreadMasters.setColumnHidden(column, True)
...
# Function to add a new line
    def add_new(self):
        """
        Inserts a new empty entry into the thread_masters table.
        Commits the changes to the database and handles any errors.
        """
        commands_new=("""
                        INSERT INTO verification.thread_masters (number_item)
                        VALUES(%s)
                        """)
        conn = None
        try:
...
# Function to adjust table size
    def adjust_table(self):
        """
        Adjusts column visibility and resize behavior in the tableThreadMasters widget.
        """
        self.tableThreadMasters.horizontalHeader().setSectionResizeMode(QtWidgets.QHeaderView.ResizeMode.ResizeToContents)
        self.tableThreadMasters.horizontalHeader().setSectionResizeMode(2,QtWidgets.QHeaderView.ResizeMode.Stretch)
        self.tableThreadMasters.horizontalHeader().setSectionResizeMode(3,QtWidgets.QHeaderView.ResizeMode.Stretch)
        self.tableThreadMasters.horizontalHeader().setSectionResizeMode(7,QtWidgets.QHeaderView.ResizeMode.Stretch)
        self.tableThreadMasters.verticalHeader().setSectionResizeMode(QtWidgets.QHeaderView.ResizeMode.ResizeToContents)
        self.tableThreadMasters.verticalHeader().setVisible(False)
        self.tableThreadMasters.hideColumn(8)
...
# Function to delete register of database
    def delete_register(self):
        ...
        """
        selection_model = self.tableThreadMasters.selectionModel()

        if not selection_model.hasSelection():
            return

        model = self.tableThreadMasters.model()
        ...
        if len(id_values) != 0:
            ...
            if result == QtWidgets.QMessageBox.StandardButton.Yes:
                ...
                # execution of commands
                    commands_delete = ("""DELETE FROM verification.thread_masters
                                        WHERE number_item = %s""")
    ...
    def add_pdf(self):
        ...
        if len(selected_indexes) == 1:
            ...
            if index.column() in [2, 3]:
                ...
                if pdf_path:
                    commands_insert_1 = ("""
                            UPDATE verification."thread_masters"
                            SET "certificate_1" = %s
                            WHERE "number_item" = %s
                            """)
...
# Function to open menu to export data
    def export_data(self):
        ...
        if self.proxy.rowCount() == 0:
            ...
            visible_columns = [col for col in range(self.model.columnCount()) if not self.tableThreadMasters.isColumnHidden(col)]
...
# Function to export data to pdf
    def exporttopdf(self, dataframe):
        ...
        pdf.cell(4.5, 1, "INSTRUMENTO", border=1, align='C')
        pdf.cell(2.5, 1, "CERTIFICADO", border=1, align='L')
        ...
        for i in range(dataframe.shape[0]):
            ...
            pdf.cell(4.5, 0.5, dataframe.iloc[i, 1], border=1, align='L')
            pdf.cell(1.25, 0.5, dataframe.iloc[i, 2], border='LTB', align='L')
            pdf.cell(1.25, 0.5, dataframe.iloc[i, 3], border='RTB', align='L')
        ...
        pdf.cell(4.5, 0.5, "RPC-001-02", border=1, align='C')
        pdf.cell(7.5, 0.5, "")
...
if __name__ == "__main__":
    ...
    Workshop_Thread_Masters_Window = Ui_Workshop_Thread_Masters_Window(db, 'm.gil')
    Workshop_Thread_Masters_Window.showMaximized()
    sys.exit(app.exec())