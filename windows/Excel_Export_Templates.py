...
LEVEL_COLUMNS_DROP_MAP = {
            "LEVEL GAUGES ELEMENTS DATA": [
                "dv_size", "dv_rating", "dv_facing", "float_material"
            ],
            "MAGNETIC ELEMENTS DATA": [
                "dv_conn", "item_type", "valve_type",
                "case_cover_material", "illuminator", "ip_code"
            ]
            }

LEVEL_VALUE_TYPE_GROUPS = {
    1: ["Transparent", "Reflex"],
    2: ["Magnetic"],
}

LEVEL_VALUE_TYPE_MAP = {
    key: group
    ...
    def __init__(self, numoffer, username, rev, project, delivery_term, delivery_time, validity, pay_term, testinspection, revchanges, notes):
        ...
        try:
            ...
            if len(data_tags_flow) == 0 or len(data_tags_temp) == 0:
                ...
                # Loading Excel Template
                self.wb_commercial = load_workbook(str(get_path("Plantillas Exportación", "PLANTILLA OFERTA CAUDAL-TEMPERATURA.xlsx")))
    ...
    def __init__(self, numoffer, username, rev, project, delivery_term, delivery_time, validity, pay_term, testinspection, revchanges, notes):
        ...
        try:
            ...
            if len(data_tags_flow) == 0 or len(data_tags_temp) == 0 or len(data_tags_level) == 0:
                ...
                # Loading Excel Template
                self.wb_commercial = load_workbook(str(get_path("Plantillas Exportación", "PLANTILLA OFERTA CAUDAL-TEMPERATURA-NIVEL.xlsx")))
...
class nuclear_annexes:
    ...
    def __init__(self, annex, numorder, ana_code, ana_order, line):
        ...
        if self.annex == 'Anexo A':
            self.wb = load_workbook(str(get_path("Plantillas Exportación", "FORMATOS ANEXOS", "ANEXO A Termopar con union a masa.xlsx")))
        elif self.annex == 'Anexo B':
            self.wb = load_workbook(str(get_path("Plantillas Exportación", "FORMATOS ANEXOS", "ANEXO B Termopar aislado de masa.xlsx")))
        elif self.annex == 'Anexo C':
            self.wb = load_workbook(str(get_path("Plantillas Exportación", "FORMATOS ANEXOS", "ANEXO C RTD.xlsx")))
...
class order_reports:
    ...
    # Filling cells with data from orders P- dataframe
        for index, row in self.df_orders_P.iterrows():  # Data in desired row
            for col_num, value in enumerate(row, start=1):
                cell = ws.cell(row=last_row, column=col_num)
                if col_num in [7, 8, 15, 16, 17, 19, 22, 24, 26, 27, 28, 30]:
                    if value is not None:
        ...
        ws.cell(row=last_row, column=37).value = f"=SUM({get_column_letter(37)}{11}:{get_column_letter(37)}{last_row - 1})"
        ws.cell(row=last_row, column=37).style = currency_style
        ws.cell(row=last_row, column=37).font = Font(bold=True, size=12)
    ...
    # Filling cells with data from orders PA- dataframe
        ...
        ws.cell(row=last_row, column=20).value = f"=SUM({get_column_letter(20)}{first_row}:{get_column_letter(20)}{last_row - 1})"
        ws.cell(row=last_row, column=20).style = currency_style
        ws.cell(row=last_row, column=20).font = Font(bold=True, size=12)
    ...
    # Filling cells with data from orders sent dataframe
        ...
        ws.cell(row=last_row, column=16).value = f"=SUM({get_column_letter(16)}{first_row}:{get_column_letter(16)}{last_row - 1})"
        ws.cell(row=last_row, column=16).style = currency_style
        ws.cell(row=last_row, column=16).font = Font(bold=True, size=12)
