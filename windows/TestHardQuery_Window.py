# Form implementation generated from reading ui file 'TestHardQuery_Window.ui'
#
...
class Ui_TestHardQuery_Window(QtWidgets.QMainWindow):
    """
    UI class for the Query Test Hard window.
    """
    def __init__(self, username):
        """
        Initializes the Ui_TestHardQuery_Window with the specified username.
    ...
    def setupUi(self, TestHardQuery_Window):
        """
        Sets up the user interface for the TestHardQuery_Window.

        Args:
            TestHardQuery_Window (QtWidgets.QMainWindow): The main window for the UI setup.
        """
        TestHardQuery_Window.setObjectName("TestHardQuery_Window")
        TestHardQuery_Window.resize(400, 561)
        TestHardQuery_Window.setMinimumSize(QtCore.QSize(600, 575))
        icon = QtGui.QIcon()
        icon.addPixmap(QtGui.QPixmap(os.path.abspath(os.path.join(basedir, "Resources/Iconos/icon.ico"))), QtGui.QIcon.Mode.Normal, QtGui.QIcon.State.Off)
        TestHardQuery_Window.setWindowIcon(icon)
        TestHardQuery_Window.setStyleSheet(
"QPushButton {\n"
"background-color: #33bdef;\n"
"  border: 1px solid transparent;\n"
"  border-radius: 3px;\n"
...
"}")
        ...
        TestHardQuery_Window.setCentralWidget(self.centralwidget)
        self.menubar = QtWidgets.QMenuBar(parent=TestHardQuery_Window)
        self.menubar.setGeometry(QtCore.QRect(0, 0, 400, 22))
        self.menubar.setObjectName("menubar")
        TestHardQuery_Window.setMenuBar(self.menubar)
        self.statusbar = QtWidgets.QStatusBar(parent=TestHardQuery_Window)
        self.statusbar.setObjectName("statusbar")
        TestHardQuery_Window.setStatusBar(self.statusbar)
...
# Function to translate and updates the text of various UI elements
    def retranslateUi(self, TestHardQuery_Window):
        """
        Translates and updates the text of various UI elements.
        """
        _translate = QtCore.QCoreApplication.translate
        TestHardQuery_Window.setWindowTitle(_translate("TestHardQuery_Window", "Prueba Dureza"))
    ...
    def querytags(self):
        ...
        self.tableTags.setRowCount(0)
        query_tags = ("""
                        SELECT id_tag_flow, num_order, tag, item_type, of_drawing, NULL as of_sensor_drawing, TO_CHAR(hard_date, 'DD/MM/YYYY'), hard_hardness, hard_hardness_hb, hard_ball, hard_force, hard_hn, hard_state, hard_obs, 1 FROM tags_data.tags_flow WHERE hard_date IS NOT NULL AND EXTRACT(YEAR FROM hard_date) IN (EXTRACT(YEAR FROM CURRENT_DATE), EXTRACT(YEAR FROM CURRENT_DATE) - 1)
                        UNION
                        SELECT id_tag_temp, num_order, tag, item_type, of_drawing, of_sensor_drawing, TO_CHAR(hard_date, 'DD/MM/YYYY'), hard_hardness, hard_hardness_hb, hard_ball, hard_force, hard_hn, hard_state, hard_obs, 1 FROM tags_data.tags_temp WHERE hard_date IS NOT NULL AND EXTRACT(YEAR FROM hard_date) IN (EXTRACT(YEAR FROM CURRENT_DATE), EXTRACT(YEAR FROM CURRENT_DATE) - 1)
                        UNION
                        SELECT id_tag_level, num_order, tag, item_type, of_drawing, NULL as of_sensor_drawing, TO_CHAR(hard_date, 'DD/MM/YYYY'), hard_hardness, hard_hardness_hb, hard_ball, hard_force, hard_hn, hard_state, hard_obs, 1 FROM tags_data.tags_level WHERE hard_date IS NOT NULL AND EXTRACT(YEAR FROM hard_date) IN (EXTRACT(YEAR FROM CURRENT_DATE), EXTRACT(YEAR FROM CURRENT_DATE) - 1)
                        UNION
                        SELECT id_tag_others, num_order, tag, description, of_drawing, NULL as of_sensor_drawing, TO_CHAR(hard_date, 'DD/MM/YYYY'), hard_hardness, hard_hardness_hb, hard_ball, hard_force, hard_hn, hard_state, hard_obs, 1 FROM tags_data.tags_others WHERE hard_date IS NOT NULL AND EXTRACT(YEAR FROM hard_date) IN (EXTRACT(YEAR FROM CURRENT_DATE), EXTRACT(YEAR FROM CURRENT_DATE) - 1)
                        UNION
                        SELECT id, num_order, tag, item_type, NULL as of_drawing, NULL as of_sensor_drawing, TO_CHAR(test_date, 'DD/MM/YYYY'), hardness, hardness_hb, ball, force, heat_number, test_state, obs, quantity FROM verification.test_hardness WHERE EXTRACT(YEAR FROM test_date) IN (EXTRACT(YEAR FROM CURRENT_DATE), EXTRACT(YEAR FROM CURRENT_DATE) - 1)
                        """)
        conn = None
        try:
        # read the connection parameters
...
#Function when clicking on table header
    ...
    def edit_data(self, item):
        ...
        if num_order in ['ALMACÉN', 'ALMACEN', 'INTERNO', 'PROTOTIPOS']:
            from windows.TestHardEdit_Window import Ui_TestHardEdit_Window
    ...
    def countSelectedCells(self):
        ...
        if len(self.tableTags.selectedIndexes()) > 1:
            ...
            sum_value = sum([(float(ix.data()) if (ix.data() is not None and ix.data().replace(',', '.', 1).replace('.', '', 1).isdigit() and ix.column() == 14) else 0) for ix in self.tableTags.selectedIndexes()])
...
#     TestHardQuery_Window = Ui_TestHardQuery_Window()
#     TestHardQuery_Window.show()
#     sys.exit(app.exec())