...
class Ui_Workshop_Calibers_Window(QtWidgets.QMainWindow):
    """
    Main window class for managing workshop calibers, including data filtering, sorting,
    and handling caliber-related operations.
    """
    def __init__(self, db, username):
        """
        Initializes the workshop calibers window, sets up the data model, proxy model, 
    ...
    def setupUi(self, Workshop_Calibers_Window):
        """
        Sets up the user interface for the Workshop_Calibers_Window.

        Args:
            Workshop_Calibers_Window (QtWidgets.QMainWindow): The main window for the UI setup.
        """
        Workshop_Calibers_Window.setObjectName("Workshop_Calibers_Window")
        Workshop_Calibers_Window.resize(790, 595)
        Workshop_Calibers_Window.setMinimumSize(QtCore.QSize(790, 595))
        icon = QtGui.QIcon()
        icon.addPixmap(QtGui.QPixmap(os.path.abspath(os.path.join(basedir, "Resources/Iconos/icon.ico"))), QtGui.QIcon.Mode.Normal, QtGui.QIcon.State.Off)
        Workshop_Calibers_Window.setWindowIcon(icon)
        if self.username == 'm.gil':
            Workshop_Calibers_Window.setStyleSheet(
            ...
            "}\n"
            "QMenu::item:selected {background-color: rgb(3, 174, 236);}")
        self.centralwidget = QtWidgets.QWidget(parent=Workshop_Calibers_Window)
        if self.username == 'm.gil':
        ...
        self.tableCalibers=QtWidgets.QTableView(parent=self.frame)
        self.model = EditableTableModel()
        self.tableCalibers.setObjectName("tableCalibers")
        self.gridLayout_2.addWidget(self.tableCalibers, 2, 0, 1, 1)
        ...
        Workshop_Calibers_Window.setCentralWidget(self.centralwidget)
        self.menubar = QtWidgets.QMenuBar(parent=Workshop_Calibers_Window)
        self.menubar.setGeometry(QtCore.QRect(0, 0, 790, 22))
        self.menubar.setObjectName("menubar")
        Workshop_Calibers_Window.setMenuBar(self.menubar)
        self.statusbar = QtWidgets.QStatusBar(parent=Workshop_Calibers_Window)
        self.statusbar.setObjectName("statusbar")
        Workshop_Calibers_Window.setStatusBar(self.statusbar)

        self.retranslateUi(Workshop_Calibers_Window)
        QtCore.QMetaObject.connectSlotsByName(Workshop_Calibers_Window)
        ...
        self.tableCalibers.setContextMenuPolicy(Qt.ContextMenuPolicy.ActionsContextMenu)
        self.tableCalibers.addActions([delete_action])
...
# Function to translate and updates the text of various UI elements
    def retranslateUi(self, Workshop_Calibers_Window):
        """
        Translates and updates the text of various UI elements.
        """
        _translate = QtCore.QCoreApplication.translate
        Workshop_Calibers_Window.setWindowTitle(_translate("Workshop_Calibers_Window", "Calibres Taller"))
        self.tableCalibers.setSortingEnabled(True)
...
# Function to load filtered table and setting in the window
    def query_calibers(self):
        """
        Queries the database for calibers with filter applied, configures and populates tables with the query results, 
        ...
        self.tableCalibers.setItemDelegate(AlignDelegate(self.tableCalibers))
        self.color_delegate = ColorDelegate(self)
        self.tableCalibers.setItemDelegateForColumn(5, self.color_delegate)

        if self.username == 'm.gil':
            self.tableCalibers.setStyleSheet("gridline-color: rgb(128, 128, 128);")
            self.tableCalibers.horizontalHeader().setStyleSheet("::section{font: 800 10pt; background-color: #33bdef; border: 1px solid white;}")
            self.tableCalibers.verticalHeader().setStyleSheet("::section{font: 10pt; background-color: #121212; border: 0.5px solid white;}")
        else:
            self.tableCalibers.horizontalHeader().setStyleSheet("::section{font: 800 10pt; background-color: #33bdef; border: 1px solid black;}")
    ...
    # Getting the unique values for each column of the model
        ...
        self.selection_model = self.tableCalibers.selectionModel()
        self.selection_model.selectionChanged.connect(self.countSelectedCells)

        self.tableCalibers.horizontalHeader().sectionDoubleClicked.connect(self.on_view_horizontalHeader_sectionClicked)
        self.tableCalibers.horizontalHeader().customContextMenuRequested.connect(self.showColumnContextMenu)
        self.tableCalibers.horizontalHeader().setContextMenuPolicy(Qt.ContextMenuPolicy.CustomContextMenu)

        self.tableCalibers.sortByColumn(1, Qt.SortOrder.AscendingOrder)

        self.tableCalibers.doubleClicked.connect(lambda index: self.item_double_clicked(index))
        
        self.adjust_table()

# Function to load complete table and setting in the window
    def query_all_calibers(self):
        """
        Queries the database for al calibers, configures and populates tables with the query results, 
        and updates the UI accordingly. Handles potential database errors and updates the UI with appropriate messages.
        """
        self.checkbox_states = {}
        self.dict_valuesuniques = {}
...
# Function when header is clicked
    def on_view_horizontalHeader_sectionClicked(self, logicalIndex):
        ...
        self.signalMapper = QtCore.QSignalMapper(self.tableCalibers)

        valuesUnique_view = []
        for row in range(self.tableCalibers.model().rowCount()):
            index = self.tableCalibers.model().index(row, self.logicalIndex)
        ...
        actionSortAscending = QtGui.QAction("Ordenar Ascendente", self.tableCalibers)
        actionSortAscending.triggered.connect(self.on_actionSortAscending_triggered)
        self.menuValues.addAction(actionSortAscending)
        actionSortDescending = QtGui.QAction("Ordenar Descendente", self.tableCalibers)
        ...
        actionDeleteFilterColumn = QtGui.QAction("Quitar Filtro", self.tableCalibers)
        ...
        actionTextFilter = QtGui.QAction("Buscar...", self.tableCalibers)
        ...
        accept_button = QtGui.QAction("ACEPTAR", self.tableCalibers)
        accept_button.triggered.connect(self.menu_acceptbutton_triggered)

        cancel_button = QtGui.QAction("CANCELAR", self.tableCalibers)
        ...
        posY = headerPos.y() + self.tableCalibers.horizontalHeader().height()
        scrollX = self.tableCalibers.horizontalScrollBar().value()
        xInView = self.tableCalibers.horizontalHeader().sectionViewportPosition(logicalIndex)
...
# Function to hide column when action clicked
    def hide_column(self):
        ...
        """
        filterColumn = self.logicalIndex
        self.tableCalibers.setColumnHidden(filterColumn, True)
...
# Function to enable copy and paste cells
    def keyPressEvent(self, event):
        ...
        if event.matches(QKeySequence.StandardKey.Copy):
            if self.tableCalibers.selectionModel() != None:
                selected_indexes = self.tableCalibers.selectionModel().selectedIndexes()
                if selected_indexes:
...
# Function to count selected cells and sum its values
    def countSelectedCells(self):
        ...
        """
        if len(self.tableCalibers.selectedIndexes()) > 1:
            ...
            sum_value = sum([self.euro_string_to_float(str(ix.data())) if re.match(r'^[\d.,]+\sÇ$', str(ix.data())) else float(str(ix.data()).replace(',', '.')) if str(ix.data()).replace(',', '.').replace('.', '', 1).isdigit() else 0 for ix in self.tableCalibers.selectedIndexes()])
            count_value = len([ix for ix in self.tableCalibers.selectedIndexes() if ix.data() != ""])
            if sum_value > 0:
...
# Function to show context menu when right-click
    def showColumnContextMenu(self, pos):
        ...
        """
        header = self.tableCalibers.horizontalHeader()
        column = header.logicalIndexAt(pos)
        self.context_menu.exec(self.tableCalibers.mapToGlobal(pos))
...
# Function to hide selected columns
    def hideSelectedColumns(self):
        ...
        for column in selected_columns:
            self.tableCalibers.setColumnHidden(column, True)
...
# Function to add a new line
    def add_new(self):
        """
        Inserts a new empty entry into the calibers_workshop table.
        Commits the changes to the database and handles any errors.
        """
        commands_new=("""
                        INSERT INTO verification.calibers_workshop (equipment_number)
                        VALUES(%s)
                        """)
        conn = None
        try:
...
# Function to adjust table size
    def adjust_table(self):
        """
        Adjusts column visibility and resize behavior in the tableCalibers widget.
        """
        self.tableCalibers.horizontalHeader().setSectionResizeMode(QtWidgets.QHeaderView.ResizeMode.ResizeToContents)
        self.tableCalibers.horizontalHeader().setSectionResizeMode(12,QtWidgets.QHeaderView.ResizeMode.Stretch)
        self.tableCalibers.verticalHeader().setSectionResizeMode(QtWidgets.QHeaderView.ResizeMode.ResizeToContents)
        self.tableCalibers.verticalHeader().setVisible(False)
        self.tableCalibers.hideColumn(13)

# Function when item is double clicked
    def item_double_clicked(self,index):
        """
        Opens detailed caliber information if the first column is double-clicked.
        """
        if index.column() == 0:
            self.open_caliber_information(index)

# Function to see revisions of caliber
    def open_caliber_information(self,index):
        """
        Opens the information window for the selected caliber by connecting to its database.
        """
        value = index.data()

        from Workshop_Calibers_Rev_Window import Ui_Workshop_Calibers_Rev_Window
        ...
        db_caliber = createConnection_name(user_database, password_database, 'caliber_'+ str(value))
        if not db_caliber:
            sys.exit()

        self.caliber_rev_window = Ui_Workshop_Calibers_Rev_Window(db_caliber, self.username, value)
        self.caliber_rev_window.showMaximized()
...
# Function to delete register of database
    def delete_register(self):
        ...
        """
        selection_model = self.tableCalibers.selectionModel()

        if not selection_model.hasSelection():
            return

        model = self.tableCalibers.model()
        ...
        if len(id_values) != 0:
            ...
            if result == QtWidgets.QMessageBox.StandardButton.Yes:
                ...
                # execution of commands
                    commands_delete = ("""DELETE FROM verification.calibers_workshop
                                        WHERE id = %s""")
...
if __name__ == "__main__":
    ...
    Workshop_Calibers_Window = Ui_Workshop_Calibers_Window(db, 'm.gil')
    Workshop_Calibers_Window.showMaximized()
    sys.exit(app.exec())