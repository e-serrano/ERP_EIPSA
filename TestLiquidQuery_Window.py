...
"}")
    ...
    def querytags(self):
        query_tags = ("""
                        SELECT num_order, tag, item_type, of_drawing, NULL as of_sensor_drawing, TO_CHAR(lp_date, 'DD/MM/YYYY'), lp_hn_liq1, lp_hn_liq2, lp_hn_liq3, lp_state, lp_obs FROM tags_data.tags_flow WHERE lp_date IS NOT NULL
                        UNION
                        SELECT num_order, tag, item_type, of_drawing, of_sensor_drawing, TO_CHAR(lp_date, 'DD/MM/YYYY'), lp_hn_liq1, lp_hn_liq2, lp_hn_liq3, lp_state, lp_obs FROM tags_data.tags_temp WHERE lp_date IS NOT NULL
                        UNION
                        SELECT num_order, tag, item_type, of_drawing, NULL as of_sensor_drawing, TO_CHAR(lp_date, 'DD/MM/YYYY'), lp_hn_liq1, lp_hn_liq2, lp_hn_liq3, lp_state, lp_obs FROM tags_data.tags_level WHERE lp_date IS NOT NULL
                        UNION
                        SELECT num_order, tag, item_type, NULL as of_drawing, NULL as of_sensor_drawing, TO_CHAR(test_date, 'DD/MM/YYYY'), hn_liq1, hn_liq2, hn_liq3, test_state, obs FROM verification.test_liquid
                        """)
        conn = None
        try:
        # read the connection parameters
