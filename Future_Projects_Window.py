...
class Ui_Future_Projects_Window(QtWidgets.QMainWindow):
    """
    ...
    def __init__(self, db):
        """
        Initializes the Ui_Future_Projects_Window with the specified name and database connection.
    ...
    def setupUi(self, Future_Projects_Window):
        """
        Sets up the user interface for the Future_Projects_Window.

        Args:
            Future_Projects_Window (QtWidgets.QMainWindow): The main window for the UI setup.
        """
        Future_Projects_Window.setObjectName("Future_Projects_Window")
        Future_Projects_Window.resize(790, 595)
        Future_Projects_Window.setMinimumSize(QtCore.QSize(900, 595))
        icon = QtGui.QIcon()
        ...
        self.tableProjects=QtWidgets.QTableView(parent=self.frame)
        self.tableProjects.setObjectName("tableProjects")
        self.gridLayout_2.addWidget(self.tableProjects, 2, 0, 1, 1)
        ...
        Future_Projects_Window.setCentralWidget(self.centralwidget)
        self.menubar = QtWidgets.QMenuBar(parent=Future_Projects_Window)
        self.menubar.setGeometry(QtCore.QRect(0, 0, 790, 22))
        self.menubar.setObjectName("menubar")
        Future_Projects_Window.setMenuBar(self.menubar)
        self.statusbar = QtWidgets.QStatusBar(parent=Future_Projects_Window)
        self.statusbar.setObjectName("statusbar")
        Future_Projects_Window.setStatusBar(self.statusbar)

        self.retranslateUi(Future_Projects_Window)
...
# Function to translate and updates the text of various UI elements
    def retranslateUi(self, Future_Projects_Window):
        """
        Translates and updates the text of various UI elements.
        """
        _translate = QtCore.QCoreApplication.translate
        Future_Projects_Window.setWindowTitle(_translate("Future_Projects_Window", "Projectos Futuros"))
...
# Function to delete all filters when tool button is clicked
    def delete_allFilters(self):
        ...
        # self.tableProjects.setModel(None)
        self.tableProjects.setModel(self.proxy)
...
# Function to query documents data into table
    def query_projects(self):
        """
        Queries the database for filtered projects, configures and populates tables with the query results, 
...
# Function to query all projects data into table
    def query_all_projects(self):
        """
        Queries the database for all projects, configures and populates tables with the query results, 
        and updates the UI accordingly. Handles potential database errors and updates the UI with appropriate messages.
        """
        self.model.dataChanged.disconnect(self.saveChanges)
...
# Function when header is clicked
    def on_view_horizontalHeader_sectionClicked(self, logicalIndex):
        ...
        self.signalMapper = QtCore.QSignalMapper(self.tableProjects)

        valuesUnique_view = []
        for row in range(self.tableProjects.model().rowCount()):
            index = self.tableProjects.model().index(row, self.logicalIndex)
        ...
        actionSortAscending = QtGui.QAction("Ordenar Ascendente", self.tableProjects)
        actionSortAscending.triggered.connect(self.on_actionSortAscending_triggered)
        self.menuValues.addAction(actionSortAscending)
        actionSortDescending = QtGui.QAction("Ordenar Descendente", self.tableProjects)
        ...
        actionDeleteFilterColumn = QtGui.QAction("Quitar Filtro", self.tableProjects)
        ...
        actionTextFilter = QtGui.QAction("Buscar...", self.tableProjects)
        ...
        accept_button = QtGui.QAction("ACEPTAR", self.tableProjects)
        accept_button.triggered.connect(self.menu_acceptbutton_triggered)

        cancel_button = QtGui.QAction("CANCELAR", self.tableProjects)
        ...
        posY = headerPos.y() + self.tableProjects.horizontalHeader().height()
        scrollX = self.tableProjects.horizontalScrollBar().value()
        xInView = self.tableProjects.horizontalHeader().sectionViewportPosition(logicalIndex)
...
# Function to order column ascending
    def on_actionSortAscending_triggered(self):
        ...
        self.tableProjects.sortByColumn(sortColumn, sortOrder)
...
# Function to enable copy and paste cells
    def keyPressEvent(self, event):
        ...
        if event.matches(QKeySequence.StandardKey.Copy):
            ...
            if self.tableProjects.selectionModel() != None:
...
# Function to count selected cells and sum its values
    def countSelectedCells(self):
        ...
        """
        if len(self.tableProjects.selectedIndexes()) > 1:
            ...
            sum_value = sum([self.euro_string_to_float(str(ix.data())) if re.match(r'^[\d.,]+\sÇ$', str(ix.data())) else float(str(ix.data()).replace(',', '.')) if str(ix.data()).replace(',', '.').replace('.', '', 1).isdigit() else 0 for ix in self.tableProjects.selectedIndexes()])

            count_value = len([ix for ix in self.tableProjects.selectedIndexes() if ix.data() != ""])
            if sum_value > 0:
...
# Function to show context menu when right-click
    def showColumnContextMenu(self, pos):
        ...
        self.context_menu.exec(self.tableProjects.mapToGlobal(pos))
...
# Function to hide selected columns
    def hideSelectedColumns(self):
        ...
        for column in selected_columns:
            self.tableProjects.setColumnHidden(column, True)
...
# Function to show all hidden columns
    def show_columns(self):
        ...
        for column in self.hiddencolumns:
            self.tableProjects.setColumnHidden(column, False)
        self.hiddencolumns.clear()

# Function to import data with an excel
    def import_data(self):
        ...
        #Importing excel file into dataframe
            df_table = pd.read_excel(fname, na_values=['N/A'], keep_default_na=False, skiprows=10)
            ...
            try:
                with Database_Connection(config()) as conn:
                    with conn.cursor() as cur:
                        ...
                        insert_query = f"INSERT INTO future_projects ({colnames_str}) VALUES ({placeholders})"
...
# Function to add a new line
    def add_new(self):
        """
        Inserts a new empty entry into the future_projects table.
        Commits the changes to the database and handles any errors.
        """
        commands_new=("""
                        INSERT INTO future_projects (end_user)
                        VALUES(%s)
                        """)
...
if __name__ == "__main__":
    ...
    Future_Projects_Window = Ui_Future_Projects_Window(db)
    Future_Projects_Window.show()
    sys.exit(app.exec())