...
class Ui_TestLiquidInsert_Window(QtWidgets.QMainWindow):
    ...
    "}")
        ...
        self.gridLayout_2.addWidget(self.label_obs, 6, 4, 1, 1)
        ...
        self.gridLayout_2.addWidget(self.obs_test, 6, 5, 1, 1)
        ...
        self.gridLayout_2.addWidget(self.label_liq1, 7, 0, 1, 1)
        ...
        self.gridLayout_2.addWidget(self.hn_liq1, 7, 1, 1, 1)
        ...
        self.gridLayout_2.addWidget(self.label_liq2, 7, 2, 1, 1)
        ...
        self.gridLayout_2.addWidget(self.hn_liq2, 7, 3, 1, 1)
        ...
        self.gridLayout_2.addWidget(self.label_liq3, 7, 4, 1, 1)
        ...
        self.gridLayout_2.addWidget(self.hn_liq3, 7, 5, 1, 1)
            ...
            if material != '':
                ...
                # connect to the PostgreSQL server
                    ...
                    if material == 'Caudal':
                        ...
                        commands_tags = f" SELECT '', {self.column_id}, tag, num_order, item_type, dim_drawing, of_drawing, TO_CHAR(lp_date, 'DD/MM/YYYY'), lp_hn_liq1, lp_hn_liq2, lp_hn_liq3, lp_state, lp_obs FROM {self.table_name} WHERE num_order LIKE UPPER ('%%'||'{self.num_order_value}'||'%%') ORDER BY {self.column_id}"
                        self.num_columns = 13
                        column_headers = ['', 'ID', 'TAG', 'Nº Pedido', 'Tipo Equipo', 'Dim.', 'OF Equipo', 'Fecha', '9PR5', '9D1B', '996PB', 'Estado', 'Observaciones']
                    elif material == 'Temperatura':
                        self.table_name = "tags_data.tags_temp"
                        self.column_id = "id_tag_temp"
                        commands_tags = f" SELECT '', {self.column_id}, tag, num_order, item_type, dim_drawing, of_sensor_drawing, of_drawing, TO_CHAR(lp_date, 'DD/MM/YYYY'), lp_hn_liq1, lp_hn_liq2, lp_hn_liq3, lp_state, lp_obs FROM {self.table_name} WHERE num_order LIKE UPPER ('%%'||'{self.num_order_value}'||'%%') ORDER BY {self.column_id}"
                        self.num_columns = 14
                        column_headers = ['', 'ID', 'TAG', 'Nº Pedido', 'Tipo Equipo', 'Dim.', 'OF Sensor', 'OF Vaina', 'Fecha', '9PR5', '9D1B', '996PB', 'Estado', 'Observaciones']
                    elif material == 'Nivel':
                        ...
                        commands_tags = f" SELECT '', {self.column_id}, tag, num_order, description, dim_drawing, of_drawing, TO_CHAR(lp_date, 'DD/MM/YYYY'), lp_hn_liq1, lp_hn_liq2, lp_hn_liq3, lp_state, lp_obs FROM {self.table_name} WHERE num_order LIKE UPPER ('%%'||'{self.num_order_value}'||'%%') ORDER BY {self.column_id}"
                        self.num_columns = 13
                        column_headers = ['', 'ID', 'TAG', 'Nº Pedido', 'Tipo Equipo', 'Dim.', 'OF Equipo', 'Fecha', '9PR5', '9D1B', '996PB', 'Estado', 'Observaciones']
    ...
    def insert(self):
        ...
        if num_order == '' or (test_date == '' or (hn_liq1 == '' or (hn_liq2 == '' or hn_liq3 == ''))):
            ...
            if self.num_columns != 0:
                ...
                if len(id_list) == 0:
                    ...
                    # execution of commands
                        for element in id_list:
                            if element[1] == '':
                                commands_liquidtest = f"UPDATE {self.table_name} SET lp_date = '{test_date}', lp_hn_liq1 = '{hn_liq1}', lp_hn_liq2 = '{hn_liq2}', lp_hn_liq3 = '{hn_liq3}', lp_state = '{test_state}', lp_obs = '{notes}' WHERE {self.column_id} = {element[0]}"
                ...
                dlg3.setWindowTitle('Líquidos Penetrantes')
                ...
                while True:
                    ...
                    if clickedButton == 1:
                        ...
                        if tag != '':
                            while True:
                                item, ok = QtWidgets.QInputDialog.getItem(self, "Líquidos Penetrantes", "Seleccióna un tipo de pieza:", self.list_parts, 0, False)
                                if ok and item:
                                    item_type = item
                                    if item_type != '':
                                        while True:
                                            ...
                                            if clickedButton3 == 1:
                                                ...
                                                if qty != '' and (qty.isdigit() and int(qty) > 0):
                                                    ...
                                                    # connect to the PostgreSQL server
                                                        ...
                                                        cur = conn.cursor()
                                                    # execution of commands
                                                        commands_insertliquidtest = ("""INSERT INTO verification.test_liquid (num_order, tag, item_type, test_date, hn_liq1, hn_liq2, hn_liq3, test_state, obs, quantity) 
                                                                        VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)""")
                                                        cur.execute(commands_insertliquidtest, (num_order, tag, item_type, test_date, hn_liq1, hn_liq2, hn_liq3, test_state, notes, qty,))
...
if __name__ == "__main__":
    ...
    TestLiquidInsert_Window = Ui_TestLiquidInsert_Window('m.gil')
