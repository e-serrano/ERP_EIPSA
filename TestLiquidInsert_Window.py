            ...
            if len(results) != 0:
                ...
                # connect to the PostgreSQL server
                    ...
                    if material == 'Caudal':
                        ...
                        commands_tags = f" SELECT {self.column_id}, tag, num_order, description, of_drawing, TO_CHAR(lp_date, 'DD/MM/YYYY'), lp_hn_liq1, lp_hn_liq2, lp_hn_liq3, lp_state, lp_obs FROM {self.table_name} WHERE num_order LIKE UPPER ('%%'||'{self.num_order_value}'||'%%') ORDER BY {self.column_id}"
                        self.num_columns = 12
                        column_headers = ['ID', 'TAG', 'Nº Pedido', 'Tipo Equipo', 'OF Equipo', 'Fecha', '9PR5', '9D1B', '996PB', 'Estado', 'Observaciones','']
