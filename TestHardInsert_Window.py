            ...
            if len(results) != 0:
                ...
                # connect to the PostgreSQL server
                    ...
                    if material == 'Caudal':
                        ...
                        commands_tags = f" SELECT {self.column_id}, tag, num_order, item_type, of_drawing, TO_CHAR(hard_date, 'DD/MM/YYYY'), hard_hardness, hard_hardness_hb, hard_ball, hard_force, hard_hn, hard_state, hard_obs FROM {self.table_name} WHERE num_order LIKE UPPER ('%%'||'{self.num_order_value}'||'%%') ORDER BY {self.column_id}"
                        self.num_columns = 14
                        column_headers = ['ID', 'TAG', 'Nº Pedido', 'Tipo Equipo', 'OF Equipo', 'Fecha', 'Dureza', 'Dureza HB', 'Bola', 'Carga', 'Colada', 'Estado', 'Observaciones','']
                    elif material == 'Temperatura':
                        self.table_name = "tags_data.tags_temp"
                        self.column_id = "id_tag_temp"
                        commands_tags = f" SELECT {self.column_id}, tag, num_order, item_type, of_sensor_drawing, of_drawing, TO_CHAR(hard_date, 'DD/MM/YYYY'), hard_hardness, hard_hardness_hb, hard_ball, hard_force, hard_hn, hard_state, hard_obs FROM {self.table_name} WHERE num_order LIKE UPPER ('%%'||'{self.num_order_value}'||'%%') ORDER BY {self.column_id}"
                        self.num_columns = 15
                        column_headers = ['ID', 'TAG', 'Nº Pedido', 'Tipo Equipo', 'OF Sensor', 'OF Vaina', 'Fecha', 'Dureza', 'Dureza HB', 'Bola', 'Carga', 'Colada', 'Estado', 'Observaciones','']
                    elif material == 'Nivel':
                        ...
                        commands_tags = f" SELECT {self.column_id}, tag, num_order, description, of_drawing, TO_CHAR(hard_date, 'DD/MM/YYYY'), hard_hardness, hard_hardness_hb, hard_ball, hard_force, hard_hn, hard_state, hard_obs FROM {self.table_name} WHERE num_order LIKE UPPER ('%%'||'{self.num_order_value}'||'%%') ORDER BY {self.column_id}"
                        self.num_columns = 14
                        column_headers = ['ID', 'TAG', 'Nº Pedido', 'Tipo Equipo', 'OF Equipo', 'Fecha', 'Dureza', 'Dureza HB', 'Bola', 'Carga', 'Colada', 'Estado', 'Observaciones','']
