...
class EditableTableModel(QtSql.QSqlTableModel):
    ...
    def flags(self, index):
        ...
        if index.column() in [0, 2]:
            if self._force_edit:
                return flags | Qt.ItemFlag.ItemIsEditable | Qt.ItemFlag.ItemIsSelectable | Qt.ItemFlag.ItemIsEnabled
            else:
...
class Ui_Warehouse_Stock_Window(QtWidgets.QMainWindow):
    """
    Main window class for managing warehouse stock, including data filtering, sorting,
    and handling master-related operations.
    """
    def __init__(self, db, username):
        """
        Initializes the warehouse stock window, sets up the data model, proxy model, 
    ...
    def setupUi(self, Warehouse_Stock_Window):
        """
        Sets up the user interface for the Warehouse_Stock_Window.

        Args:
            Warehouse_Stock_Window (QtWidgets.QMainWindow): The main window for the UI setup.
        """
        Warehouse_Stock_Window.setObjectName("Warehouse_Stock_Window")
        Warehouse_Stock_Window.resize(790, 595)
        Warehouse_Stock_Window.setMinimumSize(QtCore.QSize(790, 595))
        icon = QtGui.QIcon()
        ...
        self.tableStock=QtWidgets.QTableView(parent=self.frame)
        self.model = EditableTableModel()
        ...
        Warehouse_Stock_Window.setCentralWidget(self.centralwidget)
        self.menubar = QtWidgets.QMenuBar(parent=Warehouse_Stock_Window)
        self.menubar.setGeometry(QtCore.QRect(0, 0, 790, 22))
        self.menubar.setObjectName("menubar")
        Warehouse_Stock_Window.setMenuBar(self.menubar)
        self.statusbar = QtWidgets.QStatusBar(parent=Warehouse_Stock_Window)
        self.statusbar.setObjectName("statusbar")
        Warehouse_Stock_Window.setStatusBar(self.statusbar)

        self.retranslateUi(Warehouse_Stock_Window)
        QtCore.QMetaObject.connectSlotsByName(Warehouse_Stock_Window)
        ...
        self.tableStock.setContextMenuPolicy(Qt.ContextMenuPolicy.ActionsContextMenu)
        self.tableStock.addActions([delete_action])
...
# Function to translate and updates the text of various UI elements
    def retranslateUi(self, Warehouse_Stock_Window):
        """
        Translates and updates the text of various UI elements.
        """
        _translate = QtCore.QCoreApplication.translate
        Warehouse_Stock_Window.setWindowTitle(_translate("Warehouse_Stock_Window", "Equipos Soldadura"))
        self.tableStock.setSortingEnabled(True)
...
# Function to delete all filters when tool button is clicked
    def delete_allFilters(self):
        ...
        if self.proxy.rowCount() != 0:
            ...
            self.tableStock.setModel(None)
            self.tableStock.setModel(self.proxy)
...
# Function to load filtered table and setting in the window
    def query_stock(self):
        ...
        self.model.setTable("verification.warehouse_stock")
        ...
        self.tableStock.setObjectName("tableStock")
        self.gridLayout_2.addWidget(self.tableStock, 2, 0, 1, 1)
        self.tableStock.setSortingEnabled(False)
        self.tableStock.verticalHeader().setVisible(False)

    # Change all column names
        headers_names = ["ID", "Descripción", "Cantidad", "Ubicación"]
    ...
    # Getting the unique values for each column of the model
        ...
        self.selection_model = self.tableStock.selectionModel()
        self.selection_model.selectionChanged.connect(self.countSelectedCells)

        self.tableStock.horizontalHeader().sectionDoubleClicked.connect(self.on_view_horizontalHeader_sectionClicked)
        self.tableStock.horizontalHeader().customContextMenuRequested.connect(self.showColumnContextMenu)
        self.tableStock.horizontalHeader().setContextMenuPolicy(Qt.ContextMenuPolicy.CustomContextMenu)

        self.tableStock.sortByColumn(0, Qt.SortOrder.AscendingOrder)

        self.tableStock.doubleClicked.connect(lambda index: self.item_double_clicked(index))
...
# Function when header is clicked
    def on_view_horizontalHeader_sectionClicked(self, logicalIndex):
        ...
        self.signalMapper = QtCore.QSignalMapper(self.tableStock)

        valuesUnique_view = []
        for row in range(self.tableStock.model().rowCount()):
            index = self.tableStock.model().index(row, self.logicalIndex)
        ...
        actionSortAscending = QtGui.QAction("Ordenar Ascendente", self.tableStock)
        actionSortAscending.triggered.connect(self.on_actionSortAscending_triggered)
        self.menuValues.addAction(actionSortAscending)
        actionSortDescending = QtGui.QAction("Ordenar Descendente", self.tableStock)
        ...
        actionDeleteFilterColumn = QtGui.QAction("Quitar Filtro", self.tableStock)
        ...
        actionTextFilter = QtGui.QAction("Buscar...", self.tableStock)
        ...
        accept_button = QtGui.QAction("ACEPTAR", self.tableStock)
        accept_button.triggered.connect(self.menu_acceptbutton_triggered)

        cancel_button = QtGui.QAction("CANCELAR", self.tableStock)
        ...
        posY = headerPos.y() + self.tableStock.horizontalHeader().height()
        scrollX = self.tableStock.horizontalScrollBar().value()
        xInView = self.tableStock.horizontalHeader().sectionViewportPosition(logicalIndex)
...
# Function to hide column when action clicked
    def hide_column(self):
        ...
        """
        filterColumn = self.logicalIndex
        self.tableStock.setColumnHidden(filterColumn, True)
...
# Function to show all hidden columns
    def show_columns(self):
        ...
        for column in self.hiddencolumns:
            self.tableStock.setColumnHidden(column, False)
...
# Function to enable copy and paste cells
    def keyPressEvent(self, event):
        ...
        if event.matches(QKeySequence.StandardKey.Copy):
            if self.tableStock.selectionModel() != None:
                selected_indexes = self.tableStock.selectionModel().selectedIndexes()
                if selected_indexes:
...
# Function to count selected cells and sum its values
    def countSelectedCells(self):
        ...
        """
        if len(self.tableStock.selectedIndexes()) > 1:
            ...
            sum_value = sum([self.euro_string_to_float(str(ix.data())) if re.match(r'^[\d.,]+\sÇ$', str(ix.data())) else float(str(ix.data()).replace(',', '.')) if str(ix.data()).replace(',', '.').replace('.', '', 1).isdigit() else 0 for ix in self.tableStock.selectedIndexes()])
            count_value = len([ix for ix in self.tableStock.selectedIndexes() if ix.data() != ""])
            if sum_value > 0:
...
# Function to show context menu when right-click
    def showColumnContextMenu(self, pos):
        ...
        self.context_menu.exec(self.tableStock.mapToGlobal(pos))
...
# Function to hide selected columns
    def hideSelectedColumns(self):
        ...
        for column in selected_columns:
            self.tableStock.setColumnHidden(column, True)
...
# Function to add a new line
    def add_new(self):
        """
        Inserts a new empty entry into the warehouse_stock table.
        Commits the changes to the database and handles any errors.
        """
        commands_new=("""
                        INSERT INTO verification.warehouse_stock (description)
                        VALUES(%s)
                        """)
...
# Function to adjust table size
    def adjust_table(self):
        """
        Adjusts column visibility and resize behavior in the tableStock widget.
        """
        self.tableStock.horizontalHeader().setSectionResizeMode(QtWidgets.QHeaderView.ResizeMode.ResizeToContents)
        self.tableStock.horizontalHeader().setSectionResizeMode(1,QtWidgets.QHeaderView.ResizeMode.Stretch)
        self.tableStock.verticalHeader().setVisible(False)
...
# Function when item is double clicked
    def item_double_clicked(self,index):
        ...
        if index.column() in [2]:
            self.tableStock.setEditTriggers(QtWidgets.QAbstractItemView.EditTrigger.NoEditTriggers)
            ...
            try:
                ...
                msg.setWindowTitle("Selecciona acción")
                msg.setText(f"Valor actual: {old_value}")
                sum_btn = msg.addButton("Sumar", QtWidgets.QMessageBox.ButtonRole.AcceptRole)
                sub_btn = msg.addButton("Restar", QtWidgets.QMessageBox.ButtonRole.AcceptRole)
                cancel_btn = msg.addButton("Cancelar", QtWidgets.QMessageBox.ButtonRole.RejectRole)
                msg.exec()

                if msg.clickedButton() == cancel_btn:
                    return
                elif msg.clickedButton() == sum_btn:
                    operation = "sumar"
                else:
                    operation = "restar"

                num, ok = QtWidgets.QInputDialog.getInt(self, f"{operation.capitalize()} valor", f"Introduce número a {operation} de {old_value}:")

                if ok:
                    new_value = old_value + num if operation == "sumar" else old_value - num
                    self.model._force_edit = True
                    self.model.setData(index, new_value, Qt.ItemDataRole.EditRole)
                    self.saveChanges()
                    self.model._force_edit = False

                    self.tableStock.closePersistentEditor(index)
...
# Function to delete register of database
    def delete_register(self):
        ...
        """
        selection_model = self.tableStock.selectionModel()

        if not selection_model.hasSelection():
            return

        model = self.tableStock.model()
...
# Function to open menu to export data
    def export_data(self):
        ...
        if self.proxy.rowCount() == 0:
            ...
            visible_columns = [col for col in range(self.model.columnCount()) if not self.tableStock.isColumnHidden(col)]
...
if __name__ == "__main__":
    ...
    Warehouse_Stock_Window = Ui_Warehouse_Stock_Window(db, 'm.gil')
    Warehouse_Stock_Window.showMaximized()
    sys.exit(app.exec())