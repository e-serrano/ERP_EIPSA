...
class Ui_Workshop_Calibers_Rev_Window(QtWidgets.QMainWindow):
    """
    Main window class for managing workshop caliber reviews, including data filtering, sorting,
    and handling caliber-related operations.

    Attributes:
        model: The table model used to handle data for the caliber revisions.
        proxy: A custom proxy model for filtering and sorting the data.
        db: The database connection object.
        caliber_id: The ID of the caliber being reviewed or modified.
    ...
    """
    def __init__(self, db, username, caliber_id):
        """
        Initializes the workshop caliber revision window, setting up data models, filters,
        and UI components for a specific caliber.

        Args:
            db: Database connection object.
            username: Username of the current user.
            caliber_id: ID of the caliber being reviewed or modified.
        """
        super().__init__()
    ...
    def closeConnection(self):
        ...
        if self.db:
            self.db.close()
            del self.db
            if QtSql.QSqlDatabase.contains("caliber"):
                QtSql.QSqlDatabase.removeDatabase("caliber")

    def setupUi(self, Workshop_Calibers_Rev_Window):
        """
        Sets up the user interface for the Workshop_Calibers_Rev_Window.

        Args:
            Workshop_Calibers_Rev_Window (QtWidgets.QMainWindow): The main window for the UI setup.
        """
        Workshop_Calibers_Rev_Window.setObjectName("Workshop_Calibers_Rev_Window")
        Workshop_Calibers_Rev_Window.resize(790, 595)
        Workshop_Calibers_Rev_Window.setMinimumSize(QtCore.QSize(790, 595))
        icon = QtGui.QIcon()
    ...
    "}")
        ...
        self.label_type.setMinimumSize(QtCore.QSize(40, 30))
        ...
        self.label_location.setMinimumSize(QtCore.QSize(40, 30))
        self.label_location.setObjectName("label_location")
        font = QtGui.QFont()
        font.setPointSize(10)
        font.setBold(True)
        self.label_location.setFont(font)
        self.gridLayout_2.addWidget(self.label_location, 2, 2, 1, 1)
        ...
        self.label_range_caliber = QtWidgets.QLabel(parent=self.frame)
        self.label_range_caliber.setMinimumSize(QtCore.QSize(40, 30))
        self.label_range_caliber.setObjectName("label_range_caliber")
        font = QtGui.QFont()
        font.setPointSize(10)
        font.setBold(True)
        self.label_range_caliber.setFont(font)
        self.gridLayout_2.addWidget(self.label_range_caliber, 3, 0, 1, 1)

        self.label_precision_caliber = QtWidgets.QLabel(parent=self.frame)
        self.label_precision_caliber.setMinimumSize(QtCore.QSize(40, 30))
        self.label_precision_caliber.setObjectName("label_precision_caliber")
        font = QtGui.QFont()
        font.setPointSize(10)
        font.setBold(True)
        self.label_precision_caliber.setFont(font)
        self.gridLayout_2.addWidget(self.label_precision_caliber, 3, 1, 1, 1)

        self.label_brand = QtWidgets.QLabel(parent=self.frame)
        self.label_brand.setMinimumSize(QtCore.QSize(40, 30))
        self.label_brand.setObjectName("label_brand")
        font = QtGui.QFont()
        font.setPointSize(10)
        font.setBold(True)
        self.label_brand.setFont(font)
        self.gridLayout_2.addWidget(self.label_brand, 3, 2, 1, 1)
        ...
        self.label_box_label = QtWidgets.QLabel(parent=self.frame)
        self.label_box_label.setMinimumSize(QtCore.QSize(40, 30))
        self.label_box_label.setObjectName("label_box_label")
        font = QtGui.QFont()
        font.setPointSize(10)
        font.setBold(True)
        self.label_box_label.setFont(font)
        self.gridLayout_2.addWidget(self.label_box_label, 4, 0, 1, 2)
        ...
        Workshop_Calibers_Rev_Window.setCentralWidget(self.centralwidget)
        self.menubar = QtWidgets.QMenuBar(parent=Workshop_Calibers_Rev_Window)
        self.menubar.setGeometry(QtCore.QRect(0, 0, 790, 22))
        self.menubar.setObjectName("menubar")
        Workshop_Calibers_Rev_Window.setMenuBar(self.menubar)
        self.statusbar = QtWidgets.QStatusBar(parent=Workshop_Calibers_Rev_Window)
        self.statusbar.setObjectName("statusbar")
        Workshop_Calibers_Rev_Window.setStatusBar(self.statusbar)

        self.retranslateUi(Workshop_Calibers_Rev_Window)
        QtCore.QMetaObject.connectSlotsByName(Workshop_Calibers_Rev_Window)
...
# Function to translate and updates the text of various UI elements
    def retranslateUi(self, Workshop_Calibers_Rev_Window):
        """
        Translates and updates the text of various UI elements.
        """
        _translate = QtCore.QCoreApplication.translate
        Workshop_Calibers_Rev_Window.setWindowTitle(_translate("Workshop_Calibers_Rev_Window", "Revisiones Calibres Taller"))
        self.Button_Image.setText(_translate("Workshop_Calibers_Rev_Window", "Ver Imagen"))
...
# Function to load table and setting in the window
    def query_revisions(self):
        """
        Queries the database for calibers revisiones, configures and populates tables with the query results, 
        ...
        self.model.setTable("verification.calibers_workshop_revisions")
        self.model.setFilter(f"caliber_id = {self.caliber_id}")
    ...
    # Change all column names
        headers_names = ["ID", "Nº Calibre", "Prueba", "Patrón 1 (0%)", "Medida 1 (0%)", "Equipo 1 (0%)",
                        "Patrón 2 (100%)", "Medida 2 (100%)", "Equipo 2 (100%)", "Patrón 3 (50%)", "Medida 3 (50%)", "Equipo 3 (50%)",
                        "Patrón 4 (75%)", "Medida 4 (75%)", "Equipo 4 (75%)", "Patrón 5 (25%)", "Medida 5 (25%)", "Equipo 5 (25%)",
...
# Function to add a new line
    def add_new(self):
        """
        Inserts a new empty entry into the calibers_workshop_revisions table.
        Commits the changes to the database and handles any errors.
        """
        commands_new=("""
                        INSERT INTO verification.calibers_workshop_revisions (caliber_id)
                        VALUES(%s)
                        """)
        conn = None
        try:
...
# Function to add images to calibers
    def add_images(self):
        """
        ...
        if images_path:
            commands_insert = ("""
                    UPDATE verification."calibers_workshop"
                    SET "image" = %s
                    WHERE "id" = %s
                    """)
            ...
            # execution of commands
                cur.execute(commands_insert, (images_path, self.caliber_id,))
            ...
            # commit the changes
                ...
                new_icon.addPixmap(QtGui.QPixmap(os.path.abspath(os.path.join(basedir, "Resources/Iconos/icon.ico"))), QtGui.QIcon.Mode.Normal, QtGui.QIcon.State.Off)
                dlg.setWindowIcon(new_icon)
                dlg.setWindowTitle("Calibres Taller")
                dlg.setText("Ha ocurrido el siguiente error:\n"
                            + str(error))
                dlg.setIcon(QtWidgets.QMessageBox.Icon.Critical)
                dlg.exec()
...
# Function to add information to calibers
    def save_information(self):
        """
        Saves or updates the characteristics of the selected caliber in the database.
        Updates the 'characteristics' field for the caliber using data entered in the text field.
        """
        commands_insert = ("""
                UPDATE verification."calibers_workshop"
        ...
        # connect to the PostgreSQL server
            ...
            cur = conn.cursor()
        # execution of commands
            cur.execute(commands_insert, (self.caliber_characteristics.toPlainText(), self.caliber_id,))
...
# Function to load data
    def load_data(self):
        """
        Loads caliber data from the database and updates the UI components.
        Displays the data in relevant UI elements.
        """
        query_caliber_data = ("""SELECT equipment_number, type_caliber, location_caliber, TO_CHAR(last_check_date,'DD/MM/YYYY'), TO_CHAR(next_check_date,'DD/MM/YYYY'),
                            range_caliber, precision_caliber, master, brand,
                            reference, box_label, notes FROM verification.calibers_workshop WHERE id = %s""")
        ...
        # connect to the PostgreSQL server
            ...
            cur = conn.cursor()
        # execution of commands
            cur.execute(query_caliber_data, (self.caliber_id,))
        ...
        self.label_number.setText("NÚMERO: " + results_caliber[0][0] if results_caliber[0][0] is not None else "")
        self.label_type.setText("TIPO: " + results_caliber[0][1] if results_caliber[0][1] is not None else "")
        self.label_location.setText("UBICACIÓN: " + results_caliber[0][2] if results_caliber[0][2] is not None else "")
        self.label_last_check.setText("ÚLTIMA COMP.: " + results_caliber[0][3] if results_caliber[0][3] is not None else "")
        self.label_next_check.setText("PROX. COMP.: " + results_caliber[0][4] if results_caliber[0][4] is not None else "")
        self.label_range_caliber.setText("RANGO: " + results_caliber[0][5] if results_caliber[0][5] is not None else "")
        self.label_precision_caliber.setText("PRECISION: " + results_caliber[0][6] if results_caliber[0][6] is not None else "")
        self.label_master.setText("PATRÓN: " + results_caliber[0][7] if results_caliber[0][7] is not None else "")
        self.label_brand.setText("MARCA: " + results_caliber[0][8] if results_caliber[0][8] is not None else "")
        self.label_reference.setText("REFERENCIA: " + results_caliber[0][9] if results_caliber[0][9] is not None else "")
        self.label_notes.setText(results_caliber[0][11] if results_caliber[0][11] is not None else "")
        self.label_box_label.setText("CAJA/ETIQ.: " + results_caliber[0][10] if results_caliber[0][10] is not None else "")
        ...
        if len(id_values) != 0:
            ...
            if result == QtWidgets.QMessageBox.StandardButton.Yes:
                ...
                # connect to the PostgreSQL server
                    ...
                    cur = conn.cursor()
                # execution of commands
                    commands_delete = ("""DELETE FROM verification.calibers_workshop_revisions
                                        WHERE id = %s""")
                ...
                # commit the changes
                    ...
                    new_icon.addPixmap(QtGui.QPixmap(os.path.abspath(os.path.join(basedir, "Resources/Iconos/icon.ico"))), QtGui.QIcon.Mode.Normal, QtGui.QIcon.State.Off)
                    dlg.setWindowIcon(new_icon)
                    dlg.setWindowTitle("Revisiones Calibres")
...
# Function to correct image orientation
    def open_image(self):
        ...
        """
        query_path_image = ("""SELECT image FROM verification.calibers_workshop WHERE id = %s""")
        ...
        # connect to the PostgreSQL server
            ...
            cur = conn.cursor()
        # execution of commands
            cur.execute(query_path_image, (self.caliber_id,))
...
# Function to print pdf
    def datasheet_pdf(self):
        """
        Generates a PDF datasheet for the caliber with details and revisions.
        Opens the generated PDF in the viewer.
        """
        query_caliber_data = ("""SELECT equipment_number, reference, type_caliber, brand, range_caliber, precision_caliber,
                                master, 'certificado' as certificate, TO_CHAR(last_check_date, 'DD/MM/YYYY'),TO_CHAR(next_check_date, 'DD/MM/YYYY'), notes, image
                                FROM verification.calibers_workshop WHERE id = %s""")
        query_caliber_revision = ("""SELECT * FROM verification.calibers_workshop_revisions WHERE caliber_id = %s""")
        ...
        # execution of commands
            ...
            cur.execute(query_caliber_revision, (self.caliber_id,))
        ...
        if len(results_caliber) != 0:
            ...
            pdf.set_font('Helvetica', '', 9)
            pdf.cell(7, 0.5, str(results_caliber[0][0]), border=1, align='L')
            pdf.ln(0.5)
            pdf.set_font('Helvetica', 'B', 9)
            pdf.cell(4, 0.5, "Nº SERIE", border=1, align='L', fill=True)
            pdf.set_font('Helvetica', '', 9)
            pdf.cell(7, 0.5, str(results_caliber[0][1]), border=1, align='L')
            pdf.ln(0.5)
            pdf.set_font('Helvetica', 'B', 9)
            pdf.cell(4, 0.5, "TIPO", border=1, align='L', fill=True)
            pdf.set_font('Helvetica', '', 9)
            pdf.cell(7, 0.5, str(results_caliber[0][2]), border=1, align='L')
            pdf.ln(0.5)
            pdf.set_font('Helvetica', 'B', 9)
            pdf.cell(4, 0.5, "FABRICANTE", border=1, align='L', fill=True)
            pdf.set_font('Helvetica', '', 9)
            pdf.cell(7, 0.5, str(results_caliber[0][3]), border=1, align='L')
            pdf.ln(0.5)
            pdf.set_font('Helvetica', 'B', 9)
            pdf.cell(4, 0.5, "RANGO", border=1, align='L', fill=True)
            pdf.set_font('Helvetica', '', 9)
            pdf.cell(7, 0.5, str(results_caliber[0][4]), border=1, align='L')
            pdf.ln(0.5)
            pdf.set_font('Helvetica', 'B', 9)
            pdf.cell(4, 0.5, "PRECISIÓN", border=1, align='L', fill=True)
            pdf.set_font('Helvetica', '', 9)
            pdf.cell(7, 0.5, str(results_caliber[0][5]), border=1, align='L')
            pdf.ln(0.5)
            pdf.set_font('Helvetica', 'B', 9)
            ...
            pdf.set_font('Helvetica', '', 9)
            pdf.cell(7, 0.5, str(results_caliber[0][6]), border=1, align='L')
            pdf.ln(0.5)
            pdf.set_font('Helvetica', 'B', 9)
            pdf.cell(4, 0.5, "CERTIFICADO PATRÓN", border=1, align='L', fill=True)
            pdf.set_font('Helvetica', '', 9)
            pdf.cell(7, 0.5, str(results_caliber[0][7]), border=1, align='L')
            pdf.ln(0.5)
            pdf.set_font('Helvetica', 'B', 9)
            pdf.cell(4, 0.5, "FECHA REALIZACIÓN", border=1, align='L', fill=True)
            pdf.set_font('Helvetica', '', 9)
            pdf.cell(7, 0.5, str(results_caliber[0][8]), border=1, align='L')
            pdf.ln(0.5)
            pdf.set_font('Helvetica', 'B', 9)
            pdf.cell(4, 0.5, "PRÓXIMA CALIBRACIÓN", border=1, align='L', fill=True)
            pdf.set_font('Helvetica', '', 9)
            pdf.cell(7, 0.5, str(results_caliber[0][9]), border=1, align='L')
            pdf.ln(0.5)
            ...
            pdf.ln(1.5)
            pdf.cell(2.5, 0.5, 'CER-' + results_caliber[0][0], border=1, align='C')
            ...
            dlg_error.setWindowTitle("Calibres")
...
if __name__ == "__main__":
    ...
    Workshop_Calibers_Rev_Window = Ui_Workshop_Calibers_Rev_Window(db, 'm.gil', '30')
    Workshop_Calibers_Rev_Window.showMaximized()
    sys.exit(app.exec())